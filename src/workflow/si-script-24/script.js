...
var display_name =
  workflow.inputs.u_lastname + ', ' + workflow.inputs.u_firstname;
...
if (workflow.inputs.u_title) user.title = String(workflow.inputs.u_title);
...
if (workflow.inputs.u_manager.user_name) {
  ...
  user.Manager =
    'CN=' +
    workflow.inputs.u_manager.user_name +
    ',OU=Time Warner Telecom,DC=ad,DC=twtelecom,DC=com';
}
...
if (workflow.inputs.u_group) {
  ...
  for (var i = 0; i < arr.length; i++) {
    //str holds sys_id, so need to find name
  ...
  workflow.debug(desc);
} else workflow.scratchpad.processgroups = false;
